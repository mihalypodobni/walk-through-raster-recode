...
function addClass(onClick) {
    ...
    for (let i = 1; i <= arrayLength; i++) {       //cycles through all the divs
        ...
        if (onClick) {
            delay=12*i;
            classWriteDelay(i,stroke,bgColor,delay); 
        } else {
            delay=0;
            classWriteDelay(i,stroke,bgColor,delay); 
            }
...
function classWriteDelay(i,stroke,bgColor,delay) {
    setTimeout(function() {
...
function S (query, log, arr, index) {
...
S(`#body`).addEventListener('click', () => addClass(true))
